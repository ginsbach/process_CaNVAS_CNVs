...
def readInputs(inputfile, inputpath):
	...
	with os.scandir(inputpath) as entries:
	   for entry in entries:
	    	...
	    	leftCNV_sum.rename(columns={"logRratio": "medLRROutleft"}, inplace=True)
	    	rightCNV_sum=rightCNV.groupby(['CNVID'])['logRratio'].median()
	    	rightCNV_sum.rename(columns={"logRratio": "medLRROutright"}, inplace=True)
	    	inCNV_mean=inCNV.groupby(['CNVID'])['logRratio'].mean()
	    	inCNV_mean.rename(columns={"logRratio": "meanLRRIn"}, inplace=True)
	    	leftCNV_mean=leftCNV.groupby(['CNVID'])['logRratio'].mean()
	    	leftCNV_mean.rename(columns={"logRratio": "meanLRROutleft"}, inplace=True)
	    	rightCNV_mean=rightCNV.groupby(['CNVID'])['logRratio'].mean()
	    	rightCNV_mean.rename(columns={"logRratio": "meanLRROutright"}, inplace=True)
	    	inCNV['BAF_bins']=pd.cut(inCNV.BAF,[0,0.005,0.29,0.37,0.47,0.53,0.63,0.71,0.995,1], include_lowest=True, labels=pd.Categorical(['Homozygote',"None","Trisomic","None","Disomic","None","Trisomic","None","Homozygote"]))
